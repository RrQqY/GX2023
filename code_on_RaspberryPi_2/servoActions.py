...
class servoActions():
    ...
    def __init__(self):
        ...
        self.grasp_close_angle = 1            # 爪子闭合角度
        self.grasp_start_angle = 84
    ...
    def servo_start(self):
        ...
        start = [[70, -90, 30, self.grasp_start_angle]]
    ...
    def get_pla1(self):
        ...
        self.uservo.set_servo_angle(self.SERVO_ID_0, pla1_get[0][0],velocity=125.0 * self.decay * vel_fast, t_acc=self.acc, t_dec=self.dec)
        self.uservo.set_servo_angle(self.SERVO_ID_1, pla1_get[0][1],velocity=70.0 * self.decay * vel_fast, t_acc=self.acc, t_dec=self.dec)
        self.uservo.set_servo_angle(self.SERVO_ID_2, pla1_get[0][2],velocity=210.0 * self.decay * vel_fast, t_acc=self.acc, t_dec=self.dec)
        self.uservo.set_servo_angle(self.SERVO_ID_3, pla1_get[0][3],velocity=140.0 * self.decay * vel_fast, t_acc=self.acc, t_dec=self.dec)
        time.sleep(0.8 / self.decay / vel_fast)
    ...
    def put_pla2(self, pos):
        ...
        """
        # 放置货架② (场地右侧) 
        middle_angle = self.forward_angle - 2
        left_angle = self.forward_angle - 31
        ...
        # 太近
        # pla2_pos1_put = [[left_angle, 45, 98, -58, self.grasp_close_angle],
        #                 [left_angle, 10, 80, -62, self.grasp_close_angle],       # 放下
        #                 [left_angle, 90, 135, -65, self.grasp_open_angle]]        # 缩回
        
        # pla2_pos2_put = [[middle_angle, 50, 120, -70, self.grasp_close_angle],
        #                 [middle_angle, 20, 102, -82, self.grasp_close_angle],       # 放下
        #                 [middle_angle, 90, 135, -65, self.grasp_open_angle]]        # 缩回
        
        # pla2_pos3_put = [[right_angle, 45, 98, -58, self.grasp_close_angle],
        #                 [right_angle, 10, 80, -62, self.grasp_close_angle],       # 放下
        #                 [right_angle, 90, 135, -65, self.grasp_open_angle]]        # 缩回
        
        pla2_pos1_put = [[left_angle, 45, 98, -58, self.grasp_close_angle],
                        [left_angle, 8, 76, -62, self.grasp_close_angle],       # 放下
                        [left_angle, 90, 135, -65, self.grasp_open_angle]]        # 缩回
        
        pla2_pos2_put = [[middle_angle, 50, 120, -70, self.grasp_close_angle],
                        [middle_angle, 18, 100, -82, self.grasp_close_angle],       # 放下
                        [middle_angle, 90, 135, -65, self.grasp_open_angle]]        # 缩回
        
        pla2_pos3_put = [[right_angle, 45, 98, -58, self.grasp_close_angle],
                        [right_angle, 8, 76, -62, self.grasp_close_angle],       # 放下
                        [right_angle, 90, 135, -65, self.grasp_open_angle]]        # 缩回
    ...
    def get_pla2(self, pos):
        ...
        pla2_pos1_get = [[left_angle, 45, 135, -5, self.grasp_open_angle],
        ...
        pla2_pos3_get = [[right_angle, 45, 135, -5, self.grasp_open_angle],
        ...
        if pos == 1:
            ...
            self.uservo.set_servo_angle(self.SERVO_ID_2, pla2_pos1_get[1][3],velocity=230.0 * self.decay, t_acc=self.acc, t_dec=self.dec)
        ...
        ### 位置2
            ...
            self.uservo.set_servo_angle(self.SERVO_ID_2, pla2_pos2_get[1][3],velocity=270.0 * self.decay, t_acc=self.acc, t_dec=self.dec)
        ...
        ### 位置3
            ...
            self.uservo.set_servo_angle(self.SERVO_ID_2, pla2_pos3_get[1][3],velocity=230.0 * self.decay, t_acc=self.acc, t_dec=self.dec)
    ...
    def put_pla3_down(self, pos):
        ...
        pla3_pos1_down_put = [[left_angle, 45, 98, -58, self.grasp_close_angle],
                        [left_angle, 12, 80, -64, self.grasp_close_angle],       # 放下
                        [left_angle, 90, 135, -65, self.grasp_open_angle]]        # 缩回
        ...
        pla3_pos3_down_put = [[right_angle, 45, 98, -58, self.grasp_close_angle],
                        [right_angle, 12, 80, -64, self.grasp_close_angle],       # 放下
                        [right_angle, 90, 135, -65, self.grasp_open_angle]]        # 缩回
    ...
    def depo_to(self, color):
        ...
        depo_order_blue = self.angle_to_order(3, 240 + 5)       # 240度
