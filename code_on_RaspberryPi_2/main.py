...
def send_move_order(dx, dy):
    ...
    while True:
        ...
        Mega_uart.flush()
        etime = time.time()
        if etime - stime > receive_order_timeout:
            print('Mega uart send timeout !!!')
            break
...
def compensate(side):
    """
    在靶标前补偿校正 (1: 上侧靶标, 2: 左侧靶标(精确), 3: 左侧靶标(粗标), 4: 决赛转盘(精确))
    """
    if side == 1:
        # 发送补偿校正指令"a"
        ...
        # 发送补偿校正指令"c"
        send_compensate_order(side=4)

    compensate_timeout_time = 6.4
    compensate_timeout_time_short = 2.5
    stime = time.time()
    ...
    while cam.cap.isOpened():
        ...
        rtime = time.time()
        if side == 4:
            if rtime - stime > compensate_timeout_time_short:
                print("time out")
                break
        else:
        ...
        if frame is not None:
            ...
            print("x_bias, y_bias: ", x_bias, y_bias, "time: ")
    ...
    for i in str(target):
        ...
        if int(i) == 3:
            ...
            if seq_i == 0:
                ...
                time.sleep(delay_time)
            servo.get_pla3_down(1)


def order8(target, turn=1):
    """
    按target顺序将物块放入转盘
    """
    print("@ Start order 8")
    ...
    # color_pre = 'n'
    # while True:
    #     # color = cam.get_target_color()
    #     circle_flag = cam.get_target_circle()
    #     # if (color == 'r') or (color == 'g') or (color == 'b'):
    #     # if (color == 1):
    #     if (circle_flag == 1):
    #         color_pre = color
    #         print("#### find first color!!!!!!!!!")
    #         time.sleep(0.2)
    #         break
    ...
    while True:
        while cam.cap.isOpened():
            ...
            ret, frame = cam.cap.read()
            # color = cam.get_target_color()
            circle_flag = cam.get_target_circle(frame, side=4)
            # if (color == 1):
            if (circle_flag == 1):
                print("#### find second color!!!!!!!!!")

                compensate(side=4)     # 决赛转盘带物块的补偿
                time.sleep(1.2)
                break
        break
    ...
    while True:
        while cam.cap.isOpened():
            ...
            ret, frame = cam.cap.read()
            # color = cam.get_target_color()
            color = cam.get_target_color(frame, side=4)
            print("#### find color: ", color)
            # if (color == 1):
            if (color == 'g'):
                print("#### find green color!!!!!!!!!")
    ...
    if(kucun_stop == 3 and kucun_speed == 300):
        one_disc = 5.47
        twice_disc = 10.9
    elif(kucun_stop == 3 and kucun_speed == 400):
        one_disc = 5.38
        twice_disc = 10.8
    elif(kucun_stop == 3 and kucun_speed == 500):
        one_disc = 5.1
        twice_disc = 10.2
    elif(kucun_stop == 4 and kucun_speed == 300):
        one_disc = 6.47
        twice_disc = 12.9
    elif(kucun_stop == 4 and kucun_speed == 400):
        one_disc = 6.38
        twice_disc = 12.8
    elif(kucun_stop == 4 and kucun_speed == 500):
        one_disc = 6.1
        twice_disc = 12.2
    elif(kucun_stop == 5 and kucun_speed == 300):
        one_disc = 7.47
        twice_disc = 14.9
    elif(kucun_stop == 5 and kucun_speed == 400):
        one_disc = 7.38
        twice_disc = 14.8
    elif(kucun_stop == 5 and kucun_speed == 500):
        one_disc = 7.1
        twice_disc = 14.2

    # 当前颜色为 'g' , 根据逻辑进行处理
    if (kucun_speed == 300 and kucun_stop == 3): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
            servo.depo_to('g')
            time.sleep(one_disc-secondtime_delay) #1格
            servo.put_disc()  #执行放置和抬起工作
            servo.depo_to('b')
            time.sleep(one_disc-secondtime_delay) #1格
            servo.put_disc()  #执行放置和抬起工作
        elif (int(str(target)[0])==1 and int(str(target)[1])==3 and int(str(target)[2])==2):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
            servo.depo_to('b')
            time.sleep(twice_disc-secondtime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
            servo.depo_to('g')
            time.sleep(twice_disc-secondtime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
        elif (int(str(target)[0])==2 and int(str(target)[1])==1 and int(str(target)[2])==3):
            servo.depo_to('g')
            time.sleep(0) #0格
            servo.put_disc()  #执行放置和抬起工作
            servo.depo_to('r')
            ...
            time.sleep(twice_disc-secondtime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
        elif (int(str(target)[0])==2 and int(str(target)[1])==3 and int(str(target)[2])==1):
            servo.depo_to('g')
            time.sleep(0) #0格
            servo.put_disc()  #执行放置和抬起工作
            servo.depo_to('b')
            ...
            time.sleep(one_disc-secondtime_delay) #1格
            servo.put_disc()  #执行放置和抬起工作
        elif (int(str(target)[0])==3 and int(str(target)[1])==1 and int(str(target)[2])==2):
            servo.depo_to('b')
            time.sleep(one_disc-firsttime_delay) #1格
            servo.put_disc()  #执行放置和抬起工作
            servo.depo_to('r')
            time.sleep(one_disc-secondtime_delay) #1格
            servo.put_disc()  #执行放置和抬起工作
            ...
            time.sleep(one_disc-secondtime_delay) #1格
            servo.put_disc()  #执行放置和抬起工作
        elif (int(str(target)[0])==3 and int(str(target)[1])==2 and int(str(target)[2])==1):
            servo.depo_to('b')
            time.sleep(one_disc-firsttime_delay) #1格
            servo.put_disc()  #执行放置和抬起工作
            servo.depo_to('g')
            ...
            time.sleep(twice_disc-secondtime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
    elif (kucun_speed == 400 and kucun_stop == 3): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
    ...
    elif (kucun_speed == 500 and kucun_stop == 3): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
    ...
    elif (kucun_speed == 300 and kucun_stop == 4): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
    ...
    elif (kucun_speed == 400 and kucun_stop == 4): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
    ...
    elif (kucun_speed == 500 and kucun_stop == 4): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
    ...
    elif (kucun_speed == 300 and kucun_stop == 5): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
    ...
    elif (kucun_speed == 400 and kucun_stop == 5): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
    ...
    elif (kucun_speed == 500 and kucun_stop == 5): 
        if (int(str(target)[0])==1 and int(str(target)[1])==2 and int(str(target)[2])==3):
            servo.depo_to('r')
            time.sleep(twice_disc-firsttime_delay) #2格
            servo.put_disc()  #执行放置和抬起工作
...
def order10(target):
    ...
    决赛转盘随便放
    """
    print("@ Start order 10")
    servo.orient_to(servo.rightside_angle)        # 云台转动至左侧

    delay_one_time = 5.38 - secondtime_delay
    seq_i = 0
    for i in str(target):
        if int(i) == 1:
            delay_time = servo.depo_to('r')
            if seq_i == 0:
                time.sleep(delay_time + delay_one_time)
                seq_i += 1
            else:
                time.sleep(delay_time)
...
def turn_1(target_1):
    ...
    ### order8: 按target顺序将物块放入转盘
    order8(target_1, turn=1)
    #######
...
if __name__=='__main__':
    ...
    # lightOn()
    # servo.orient_to(servo.rightside_angle)
    # servo.compensate(pos=3)     # 决赛转盘带物块的补偿
    # time.sleep(1.2)
