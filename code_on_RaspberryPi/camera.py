...
class Camera():
    ...
    def __init__(self):
        ...
        ### 物块颜色获取部分
        # 红绿蓝三色HSV阈值
        self.color_dist = {'red': {'Lower': np.array([120, 80, 120]), 'Upper': np.array([180, 255, 255])},
              'blue': {'Lower': np.array([90, 80, 120]), 'Upper': np.array([140, 255, 255])},
              'green': {'Lower': np.array([50, 80, 120]), 'Upper': np.array([90, 255, 255])},
             }
            
        self.pos_seg_x = [0.3, 0.7]       # 物体色块在图像中位置阈值
        self.pos_seg_y = [0.3, 0.7]
        self.square_threshold = 0.3      # 检测面积占比阈值
