...
void move_dist_dx(double dx) {
  static int read_tof_flag = 0;
  static int change_speed_flag = 0;
  static int change_speed_flag_pre = 0;
...
//   double kx = dx / sqrt(pow(dx,2) + pow(dy,2));
  set_speed_target(-move_speed, move_speed, move_speed, -move_speed);
  while (1) {
    // 获取当前绝对位置
    ...
    // 如果还未到指定位置
    sub_temp = x_target - x_dist;
    if(abs(sub_temp) > dist_threshold_far) {
      change_speed_flag = 1;
    }
    else if(abs(sub_temp) > dist_threshold_close) {
      change_speed_flag = 2;
    }
    else{
      change_speed_flag = 3;
